...
_STUBS = Path(__file__).resolve().parents[1] / "toolroom" / "_stubs"
...
_HISTORY = Path(__file__).resolve().parents[2] / "tool-history"
...
def _color_probe_and_write(only: str, write: bool, on: bool) -> None:
    ...
    if write and not only:
        ...
        docs = Path(__file__).resolve().parents[2] / "docs" / "color-support.md"
